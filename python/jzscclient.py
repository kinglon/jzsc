...
class JzscClient:
    ...
    def get_jungongyanshou_beian(self, jungongyanshou_id):
        ...
        try:
            ...
            if not response.ok:
                ...
                if root['code'] != 200:
                    print("查询竣工验收备案失败，错误是：code={}".format(root['code']))
                    return error_result

                if 'data' not in root:
                    print("查询竣工验收备案失败，错误是：缺失data字段")
                    return error_result
                ...
                if jungongyanshou.actual_use_money is None:
                    jungongyanshou.actual_use_money = ''
                try:
    ...
    def get_jungongyanshou(self, jungongyanshou_id):
        ...
        try:
            uri = f'/APi/webApi/dataservice/query/project/projectFinishCheckInfoDetail?id={jungongyanshou_id}'
            url = self.host + uri
            headers = self.get_common_request_header()
            headers['Timeout'] = '30000'
            headers['V'] = '231012'
            ...
            if not response.ok:
                ...
                try:
                    if 'BDATE' in data_object and data_object['BDATE'] and float(data_object['BDATE']) > 0:
                        jungongyanshou.begin_date = datetime.fromtimestamp(
                            float(data_object['BDATE']) / 1000).strftime('%Y-%m-%d')
                except Exception:
                    jungongyanshou.begin_date = ''
                try:
                    if 'EDATE' in data_object and data_object['EDATE'] and float(data_object['EDATE']) > 0:
                        jungongyanshou.end_date = datetime.fromtimestamp(
                            float(data_object['EDATE']) / 1000).strftime('%Y-%m-%d')
                except Exception:
                    jungongyanshou.end_date = ''
                jungongyanshou.data_source = JzscClient.get_jungongyanshou_data_source(data_object)
                jungongyanshou.construct_scale = data_object['FACTSIZE']
    ...
    def get_jishuzhibiao(self, jishuzhibiao_id):
        ...
        try:
            ...
            if not response.ok:
                ...
                jishuzhibiao.enterprise_name = data_object['CORPNAME']
